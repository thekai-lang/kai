...
- **TAST (`tast`)** — `ty.rs:75` `KaiType::Temporal { inner, origin, duration }` (`@local` zero-cost, `@wallclock` heap-bearing via RFC3339), `Effect`/`EffectSet`/`DurationLit`/`TemporalOrigin` with `Display`, `fn_decl.rs:38` `TypedFnDecl { declared_effects: Option<EffectSet>, inferred_effects: EffectSet }` (least-fixed-point over SCCs, `Set<Effect>` not `bool`).
...
- **Ownership/Codegen (`ownership`/`codegen`)** — `heap.rs:93` `Temporal @wallclock` heap-bearing, `@local` delegates to inner; `types.rs:86` `Temporal` same as inner for LLVM (zero-cost); `emit/expr/mod.rs:389` `Retain` for `Temporal` inner, `fresh.rs:138` `Require`/`Observe` seed, `walk.rs:415` `Require`/`Observe` hoist, `stmt.rs:279` `Require`/`Observe` emit as `expr::emit` passthrough.
