...
### 5.3.7 Reversible Isolation and Rollback Failure Debt

**Closure Context Isolation:**
A `reversible` activation is strictly bound to the lexical execution of the declared function. 
1. **First-class Reversible Closures:** `fn() reversible { ... }` is explicitly rejected. Support for reversible closures is deferred until effect inference can securely manage transactional lifetimes across trait and function pointer boundaries.
2. **Escaping Closures:** Any `ClosureLit` declared within a `reversible` function body is treated as an independent execution node outside the transaction's lifecycle. To prevent accidental leakage of transactional effects (where a closure outlives the activation but still pushes to the parent's ledger), the effect checker rejects any closure within a `reversible` block that attempts to capture activation-bound transactional effects, and codegen strictly zeroes the `reversible_active` context during closure body emission.

**Rollback/Compensation Failure (`kai debt`):**
A catastrophic failure *during* a reverse-order unwind (e.g., a panic inside a `compensate` thunk, or a refcount underflow while dropping a snapshot environment) is a critical violation of the transactional contract. Because this triggers an immediate terminal exit (§10.1), the runtime maintains a synchronous tracking state. Upon failure, a `reversibility` debt record with `kind` set to `"rollback-failed"` (for snapshot failures) or `"compensation-failed"` (for thunk failures) and `event: "violation"` is synchronously appended to `.kai/debt.log` prior to the `exit(101)` call. This guarantees observability without requiring a global `atexit` flush mechanism.
