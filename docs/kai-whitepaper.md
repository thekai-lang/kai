...
**Status:** Draft v0.23 — pre-implementation specification
...
- **Memory hardening framework — proposed, not yet adopted.** Three pillars, scoped as a v0.0.8.6 hardening patch (small, fast, no roadmap delay) with full integration at v0.0.12 alongside `kai debt`'s Trust⟨C⟩ projection model:

  **Pillar 1 — CI regression tests for leak repros.** All isolation artifacts currently in `/tmp/opencode/leak/` (`stress_heap.kai`, `minimal.kai`, `minimal2.kai`, `test3.kai`–`test5.kai`, and the `&&` nested-case repros) become permanent fixtures under `tests/fixtures/`. Pattern matches the existing golden-IR fixture convention (v0003, v0004, v0005): source `.kai` files compile through the full pipeline, assertions on IR structure and/or JIT exit code. These run on every PR via `cargo test`, no ASan required — they catch ownership-pass regressions by assertion, not by leak count.

  **Pillar 2 — ASan gate in CI.** `ASAN_OPTIONS=detect_leaks=1` build + run for memory-sensitive crates (`kai-ownership`, `kai-codegen`, `kai-driver` test suite). Runs on every PR, not just nightly. Binary rebuilt with `CARGO_TARGET_DIR=target-asan` to avoid stale-binary bugs (lesson learned: `target/` vs `target-asan/` divergence caused a false-positive investigation session). Pass criterion: 0 leaks, 0 use-after-free, 0 double-free on the full test corpus including the new Pillar 1 fixtures.

  **Pillar 3 — Panic-path memory test.** When a Kai program hits a panic (§10.1: `require` violation, array OOB, integer overflow), heap allocations made before the trap must be released before process exit. Currently untested — the runtime exits via `kai_panic` → `process::exit(101)` without unwinding, so any allocations in the calling scope leak. Test pattern: JIT a program that allocates, then traps (e.g., `require false;` after creating a string), run under ASan, assert 0 leaks. If the runtime's exit path cannot unwind (current design), the test documents the *intentional* leak as a known limitation with a `#[ignore]` tag and a reference back to this Appendix A entry — rather than silently accepting it as correct.

  **Production standard (4 levels):**
  ...
  - **P1 — Memory stability:** on a representative workload, heap reaches steady state after warm-up; no sustained growth; retained objects have valid owners; caches/pools have stated bounds.
  - **P2 — Diagnostic quality:** a failing test must answer WHAT (allocation still alive), WHERE (source location), WHO (owner), WHEN (allocated), WHY (not released), PATH (control-flow causing retention). "Memory increased by 47 MB" is not a diagnostic.
  - **P3 — Regression resistance:** memory tests are CI gates, not manual pre-release checks. PR gate: static ownership + sanitizer + lifecycle tests + panic-path + short stress. Nightly: long stress + heap profiling + fuzzing + regression comparison.

  **v0.0.12 integration:** `kai debt`'s existing Trust⟨C⟩ projection model (WHAT=Claim, WHERE=location, WHY=Decay mechanism, WHEN=timestamp) maps almost directly to P2's diagnostic requirements. The allocation-lifecycle tracking needed for Pillar 2's heap profiling is a natural extension of `kai debt`'s ledger — not a parallel system. This is why v0.0.12 is the right home for the full implementation: the Trust infrastructure already exists, and memory diagnostics are a projection of it, just as debt is.

  **Not adopted yet.** Needs discussion: (a) whether Pillar 3's intentional-leak documentation is acceptable or whether the runtime exit path should be changed to unwind (§10.4's `reversible` unwind mechanism, once it exists at v0.0.9, may provide the answer); (b) whether ASan-on-every-PR is too expensive for the CI budget (alternative: ASan on main/nightly, Pillar 1 assertion-only on PRs); (c) whether the P2 diagnostic standard is achievable with current tooling or requires new `kai` CLI subcommands (e.g., `kai leak-report`).