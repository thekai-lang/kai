...
text = re.sub(
    r'Ty::Named\(Ident \{\s*name: "(.*?)"\.to_string\(\),\s*span: Span::new\(0, 0\),\s*\}\)',
    r'Ty::Path(vec![Ident { name: "\1".to_string(), span: Span::new(0, 0) }])',
    text
)

text = re.sub(
    r'name: Ident \{\s*name: "(.*?)"\.to_string\(\),\s*span: Span::new\(0, 0\),\s*\},',
    r'path: vec![Ident { name: "\1".to_string(), span: Span::new(0, 0) }],',
    text
)
