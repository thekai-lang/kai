...
pub(crate) fn emit_release_slot<'ctx>(
    ...
    match ty {
        ...
        KaiType::Optional(_) | KaiType::Result { .. } => {
            call_void(
                ctx,
                super::ownership_tagged::tagged_helper(ctx, ty, Helper::Release),
                &[value_ptr.into()],
            );
        }
            ...
            Helper::Retain => {
                ...
                match field_ty {
                    ...
                    other @ (KaiType::Optional(_) | KaiType::Result { .. }) => {
                        super::ownership_tagged::retain_tagged_copy(ctx, other, field_slot);
                    }
                    _ => {}
                }
            }
...
// Tagged-union (§9.9a) helpers live in `ownership_tagged.rs` (§8 split).
