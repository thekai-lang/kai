...
pub(crate) fn alloc_bytes(byte_len: usize) -> *mut u8 {
    if byte_len == 0 {
...
pub(crate) fn get_or_declare<'ctx>(ctx: &Ctx<'ctx>, name: &str, llvm: inkwell::types::FunctionType<'ctx>) -> FunctionValue<'ctx> {
...
pub(crate) const INTRINSICS: [(&str, *const ()); 16] = [
    ...
    ("kai_wallclock_now", kai_wallclock_now as *const ()),
    ("kai_wallclock_new", kai_wallclock_new as *const ()),
    (
